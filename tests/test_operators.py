...
@pytest.mark.task0_2
@given(a=st.floats(min_value=1e-5, max_value=1e5))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    sum_ls1 = sum(ls1)
    sum_ls2 = sum(ls2)
    individual_sums = sum_ls1 + sum_ls2

    combined_list = [a + b for a, b in zip(ls1, ls2)]
    sum_combined_list = sum(combined_list)

    assert_close(sum_combined_list, individual_sums)
